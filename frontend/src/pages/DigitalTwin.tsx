...
export default function DigitalTwin({
  ...
  return (
    <div className="flex flex-col gap-6 h-full min-h-[500px]">
      ...
      {/* Title Header */}
      <div>
        <h2 className="text-xl font-bold font-mono tracking-wider text-slate-100">FACILITY DIGITAL TWIN</h2>
        <p className="text-xs text-slate-400 font-mono mt-0.5">Explore real-time state vectors and trace causal dependencies.</p>
      </div>

      <div className="grid grid-cols-4 gap-6 items-stretch">
        
        {/* Left 3 Cols: Interactive React Flow Network Graph (3 Cols) */}
        <div className="col-span-3 bg-industrial-900 border border-industrial-800 p-2 rounded-sm relative flex flex-col min-h-[480px]">
          <div className="absolute top-4 left-4 z-10 bg-slate-900/90 border border-industrial-800 p-2.5 rounded-sm flex items-center gap-2 text-xs font-mono select-none">
            <Radio className="w-4 h-4 text-alarm-green animate-pulse" />
            <span>ARGUS PLANT ALPHA NETWORK MAP</span>
          </div>
        ...
        <div className="bg-industrial-900 border border-industrial-800 p-5 rounded-sm flex flex-col gap-4 overflow-y-auto max-h-[500px]">
          ...
          {selectedNodeId && selectedNodeData && (
            <div className="flex flex-col gap-4">
              ...
              <div className="flex justify-between items-start">
                ...
                <span className={`w-2.5 h-2.5 rounded-full ${
                  selectedNodeData.status === 'Healthy' || selectedNodeData.status === 'Normal' || selectedNodeData.status === 'Operational'
                    ...
                    ? 'bg-alarm-amber animate-pulse'
                    : 'bg-alarm-red animate-pulse'
                }`} />
              </div>

              {/* Node Properties details list */}
              <div className="bg-industrial-950 p-3 border border-industrial-850 rounded-sm flex flex-col gap-2 font-mono text-[10px] text-slate-400">
              ...
              {'temperature' in selectedNodeData && (
                <div className="border border-industrial-800 bg-industrial-950 p-4 rounded-sm flex flex-col gap-3">
                  ...
                  {loadingAnalysis ? (
                    ...
                    <div className="flex flex-col gap-2.5">
                      <div className="flex justify-between items-baseline font-mono">
                        <span className="text-[10px] text-slate-400 uppercase">FAILURE LIKELIHOOD</span>
                      ...
                      <div className="flex flex-col gap-2 font-mono text-[9px] text-slate-400 border-t border-industrial-850 pt-2.5">
                        ...
                        {analysis.explanations.slice(0, 4).map((exp, idx) => (
                          <div key={idx} className="flex flex-col gap-1">
                            <div className="flex justify-between">
                              <span className="truncate max-w-[110px]">{exp.feature}</span>
                            ...
                            <div className="w-full bg-slate-800 h-1.5 rounded-full overflow-hidden">
                              <div
                                className={`h-full ${exp.impact > 0 ? 'bg-alarm-red' : 'bg-alarm-green'}`}
              ...
              {selectedNodeId && selectedNodeData.status !== 'Failed' && (
                <button
                  ...
                  className="w-full py-2 bg-alarm-red/10 border border-alarm-red/45 hover:bg-alarm-red hover:text-slate-900 active:bg-alarm-red text-alarm-red rounded-sm text-[10px] font-bold font-mono tracking-widest flex items-center justify-center gap-1.5 transition-all mt-1"
                >
          ...
          {!selectedNodeId && (
            <div className="bg-industrial-950 p-6 border border-industrial-850 rounded-sm text-center flex flex-col items-center justify-center gap-2">
      ...
      <div className="bg-industrial-900 border border-industrial-800 p-4 rounded-sm flex flex-col gap-2 font-mono">
        ...
        <div className="flex items-center gap-4">
          <input
            ...
            className="flex-1 h-1.5 bg-slate-800 rounded-lg appearance-none cursor-pointer accent-alarm-purple disabled:opacity-40"
          />
          <span className="text-xs font-bold text-slate-300 w-16 text-right">T+{timeMachineTime}m</span>
        </div>
        {!hasTrajectory && (
          <span className="text-[9px] text-slate-500">Time machine scrubber unlocks automatically once a cascade simulation rollout is triggered.</span>
        )}
      </div>
